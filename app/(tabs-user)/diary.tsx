...
import {
  ...
  uploadVoiceWithQuestion,
} from "@/entities/voices/api";
import {
  Button,
  HelpButton,
  Icon,
...
export default function DiaryScreen() {
  const {
    data: randomQuestion,
    isFetching: isFetchingRandomQuestion,
    refetch: refetchRandomQuestion,
  } = useQuery(queries.questions.randomQuestion);
  ...
  const record = async () => {
    try {
      // 기존 녹음이 있으면 먼저 정리
      if (audioRecorder.uri && !recordingState.isRecording) {
        await audioRecorder.stop().catch(() => {
          // 이미 stop된 상태일 수 있으므로 에러 무시
        });
      }
  ...
  const { mutateAsync: uploadVoiceWithQuestionMutation, isPending: isSaving } =
    useMutation({
      mutationFn: async (data: UploadVoiceWithQuestionRequest) =>
        await uploadVoiceWithQuestion(data),
    });
          ...
          onSuccess: async (data: any) => {
            if (data?.success) {
              ...
              queryClient.invalidateQueries(
                queries.voices.userVoiceList(username)
              );
            } else {
  ...
  return (
    <MainLayout className="bg-gray-50">
      <MainLayout.Header>
        <MainHeader title="마음일기" rightComponent={<HelpButton />} />
      </MainLayout.Header>
      <ScrollView
            ...
            !isSaving && (
              <View className="absolute left-8">
                <Button size="lg" variant="outlined" onPress={handleReset}>
                  <Text className="text-gray90 text-[15px] font-bold px-5">
                    취소하기
                  </Text>
                </Button>
              </View>
