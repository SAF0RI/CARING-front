    ...
    return (
        <MainLayout className="bg-gray-50">
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="w-full">
                {/* 상단 감정 컴포넌트 */}
                <View className={cn(['flex-row w-full h-[172px] bg-main50 flex items-center justify-center rounded-b-[20px] px-4', bgColorMap['anxiety']])}>
                    <Icon name="anxiety" size={48} />
                    <View className="flex-col items-start justify-center mx-4 gap-y-1">
                        <Text className="text-gray90 text-[19px]">{`오늘 ${userInfo?.data?.name}님이`}</Text>
                        <Text className="text-gray90 text-[19px] font-bold">{`불안 상태에요!`}</Text>
                    </View>
                    <TouchableOpacity
                ...
                <View className="px-4">
                    ...
                    <WeeklyStatistics username={userInfo?.data?.username ?? 'care'} />
                    <View className="flex-row items-center justify-between">
                        <Text className="text-gray100 text-[20px] font-bold text-left mb-4 mt-4">
                            감정 빈도 통계
                        </Text>
